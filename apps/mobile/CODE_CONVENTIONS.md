...
- `app/`: Expo Router route files and layouts. Prefer route shims that import from `screens/*`.
- `screens/`: screen composition and route-level UI. Screens can own local visual components that are not reused elsewhere.
- `features/`: domain orchestration hooks and reusable feature pieces. Use this for query/mutation state, default-value mapping, cache invalidation, and cross-screen feature logic.
- `components/ui/`: generic UI primitives only. These should not know about pets, sitters, budget, reminders, or backend domains.
- `components/`: shared product components that may be used across screens.
...
- Relative imports are acceptable only inside the same folder or immediate feature/screen subfolder.
...
- `features/` now contains budget, pets, and sitter orchestration hooks; continue moving large screen logic incrementally.
